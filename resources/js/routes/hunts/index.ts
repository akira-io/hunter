...
comment.url = (args: { hunt: number | { id: number } } | [hunt: number | { id: number }] | number | { id: number }, options?: RouteQueryOptions) => {
    ...
    return comment.definition.url.replace('{hunt}', parsedArgs.hunt.toString()).replace(/\/+$/, '') + queryParams(options);
};
...
toggleLike.url = (
    ...
    return toggleLike.definition.url.replace('{hunt}', parsedArgs.hunt.toString()).replace(/\/+$/, '') + queryParams(options);
};
