...
export const OnlineUsers: React.FC<ChatUsersProps> = ({ currentUserId }) => {
    const { openChatWindow, createConversation, conversations } =
        useChatContext();
    ...
    const offlineFollowedHunters = followedHunters
        ...
        .filter(
            (hunter) =>
                !filteredOnlineUsers.some((user) => user.id === hunter.id),
        ) // Exclude already online users
        .filter(
            (hunter) =>
    ...
    const allUsers = [
        ...filteredOnlineUsers,
        ...offlineFollowedHunters.filter(
            (hunter) =>
                !filteredOnlineUsers.some(
                    (onlineUser) => onlineUser.id === hunter.id,
                ),
        ),
    ];
    ...
    const handleUserClick = async (userId: number) => {
        try {
            ...
            if (shouldUseMobileChat())
                return router.visit(chat.show.url(conversation.id));
                        ...
                        <div className="border-b border-zinc-200 bg-gradient-to-r px-4 py-3 dark:border-zinc-700 dark:from-zinc-800 dark:to-zinc-800">
                            ...
                            <div className="mb-3 flex items-center gap-2">
                                ...
                                <span className="ml-auto text-xs text-zinc-500 dark:text-zinc-400">
                                    ...
                                    {offlineFollowedHunters.length} offline
                                </span>
                            </div>
                            ...
                            <div className="relative">
                                <Search
                                    size={16}
                                    className="absolute top-1/2 left-3 -translate-y-1/2 transform text-zinc-400 dark:text-zinc-500"
                                />
                                <input
                                    type="text"
                            ...
                            <div className="scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-600 scrollbar-track-transparent h-60 overflow-y-auto md:h-100">
                                ...
                                {allUsers.map((item, index) => {
                                    ...
                                    const showOfflineHeader =
                                        !isOnline &&
                                        index === filteredOnlineUsers.length &&
                                        offlineFollowedHunters.length > 0;
                                    const unreadCount = getUnreadCountForUser(
                                        item.id,
                                    );
                                    ...
                                    return (
                                        <React.Fragment key={item.id}>
                                            ...
                                            >
                                                ...
                                                <div
                                                    className={`size-2 rounded-full ${isOnline ? 'animate-pulse bg-emerald-400' : 'bg-zinc-400'}`}
                                                />
                                            </button>
                                        </React.Fragment>
                                    );
                            ...
                            <div className="h-60 p-6 text-center md:h-100">
                                ...
                                {!searchTerm && !followedHuntersLoading && (
                                    <p className="mt-2 text-xs text-zinc-400 dark:text-zinc-500">
                                        Hunters online e seguidos aparecerão
                                        aqui
                                    </p>
                                )}
                            </div>
