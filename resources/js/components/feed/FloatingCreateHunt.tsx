...
export function FloatingCreateHunt() {
    ...
    return (
        <div className="fixed right-4 bottom-4 z-50 md:right-10 md:bottom-10">
            <Dialog
                open={isFloatCreateHuntOpen}
                onOpenChange={setIsFloatCreateHuntOpen}
            >
                <DialogTrigger className="hover:bg-primary-dark flex h-10 w-10 items-center justify-center rounded-full bg-primary text-white shadow-lg transition-all duration-300">
                    <Plus size={24} />
                </DialogTrigger>
                <DialogContent className="overflow-y-auto sm:max-w-2xl">
                    {/* Mobile header - similar to app layout */}
                    <div className="sticky top-0 z-10 flex items-center gap-3 border-b bg-background/95 p-4 backdrop-blur supports-[backdrop-filter]:bg-background/60 sm:hidden">
                        <button
