...
export function ProfileBg({ user }: { user: User }) {
    const sanitizedBackgroundUrl = useSanitizeImageUrl(
        user.background_image_url,
    );

    return (
        <div className="h-30 sm:h-40">
            <div className="relative flex size-full items-center justify-center overflow-hidden rounded-xl bg-muted shadow-2xl">
                <div className="absolute inset-0 flex items-center justify-center gap-2">
                    {sanitizedBackgroundUrl && (
                        <img
                            className="size-full object-cover"
                            src={sanitizedBackgroundUrl}
...
function Avatar({
    ...
    return (
        <div className="flex items-start justify-start">
            <div className="px-2">
                <div className="absolute top-10 flex size-20 overflow-hidden rounded-full border-2 border-background bg-muted shadow-2xl md:size-32">
                    <OnboardingAvatar
                ...
                <div className="flex w-full flex-col items-center justify-end gap-2 sm:pr-4">
                    ...
                    <div className="flex w-full items-end justify-end gap-2">
                        {auth.user.id === user.id ? (
                            <Button
                                className="w-full"
                                onClick={() => router.get(profile.edit.url())}
                            >
                                <EditIcon />
                                Editar Perfil
                            </Button>
...
function NoData(props: { count: number }) {
    return (
        <>
            {props.count === 0 && (
                <p className="text-center text-muted">Sem dados a apresentar</p>
            )}
        </>
    );
}
...
function About({ user }: { user: User }) {
    ...
    return (
        <Tabs
            ...
            className="w-full md:mt-4 md:flex-row"
        >
            <TabsList className="mb-4 gap-1 rounded-none !bg-transparent text-foreground md:flex-col dark:bg-none">
                {tabLists.map((tab, index) => (
                    <TabsTrigger
                        key={tab.title}
                        value={`tab-${index + 1}`}
                        className="relative flex w-full items-center justify-center justify-start after:absolute after:inset-y-0 after:start-0 after:-ms-1 after:w-0.5 hover:bg-accent hover:text-foreground data-[state=active]:bg-muted data-[state=active]:shadow-none data-[state=active]:after:bg-transparent data-[state=active]:hover:bg-accent"
                    >
                        <Icon iconNode={tab.icon} aria-hidden="true" />
                        <span> {tab.title}</span>
                    </TabsTrigger>
                ...
                <Card className="w-full max-w-2xl items-center overflow-y-auto">
                    <CardDescription className="w-full px-4">
                        ...
                        <Avatar
                            ...
                            huntingsCount={huntings.length}
                            huntsCount={hunts.data.length}
                        />
                    </CardDescription>
                </Card>
