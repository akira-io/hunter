...
export const destroy = (
    args: { academicBackground: number | { id: number } } | [academicBackground: number | { id: number }] | number | { id: number },
...
destroy.url = (
    ...
    const parsedArgs = {
        academicBackground: typeof args.academicBackground === 'object' ? args.academicBackground.id : args.academicBackground,
    };

    return (
        destroy.definition.url.replace('{academicBackground}', parsedArgs.academicBackground.toString()).replace(/\/+$/, '') + queryParams(options)
    );
};
